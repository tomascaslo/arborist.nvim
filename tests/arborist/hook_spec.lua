describe("hook", function()
  ...
  describe("global function integration", function()
    ...
    it("_arborist_hook_stop sets state to waiting", function()
      sessions.add({ name = "claude:test", bufnr = vim.api.nvim_create_buf(false, true), worktree_path = "/tmp/test" })
      -- Call directly (normally called via remote-expr, but we test the function itself)
      _G._arborist_hook_stop("/tmp/test", "sid-1")
      -- Since it uses vim.schedule, we need to process pending callbacks
